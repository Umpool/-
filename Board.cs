...
// 아까 없애버린 cellSize 대신, 우리 프로젝트에 살아있는 blockSpacing 단어로 계산식을 바꿉니다.
    // 🎯 [이동 방식 정상화]: 픽셀 대신 생성/리필과 똑같은 앵커(Anchor) 방식으로 위치를 바꿉니다.
    RectTransform rt1 = block1.GetComponent<RectTransform>();
    RectTransform rt2 = block2.GetComponent<RectTransform>();

    // 앵커 기반으로 위치 교환 및 Lerp 동작 적용
        ...
        else
        {
            ...
            if (block1 != null && block2 != null)
            {
                ...
                if (rt1 != null && rt2 != null)
                {
                    ...
                    Vector2 targetMin1 = new Vector2((float)x1 / height, (float)y1 / width);
                    Vector2 targetMax1 = new Vector2((float)(x1 + 1) / height, (float)(y1 + 1) / width);

                    Vector2 startMin2 = rt2.anchorMin;
                    Vector2 startMax2 = rt2.anchorMax;

                    Vector2 targetMin2 = new Vector2((float)x2 / height, (float)y2 / width);
                    Vector2 targetMax2 = new Vector2((float)(x2 + 1) / height, (float)(y2 + 1) / width);
...
foreach (GameObject block in matches)
{
    if (block != null)
    {
        ...
        // 2. 알아낸 주소가 정상적인 판 안쪽 범위(0 ~ width-1, 0 ~ height-1)인지 엄격하게 체크합니다.
