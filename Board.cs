...
if (InfiniteMonster.Instance != null)
{
   ...
   //데드락은 여기까지 
    ...
    public void ShutdownAndCleanupBoard()
    {
        ...
        // [게임오버 팝업 연동] 하이어라키 창의 'GAMEOVER TXT' 오브젝트를 화면에 활성화합니다.
        GameObject gameOverPanel = GameObject.Find("GAMEOVER TXT");
...
} // 🚨 파일의 맨 마지막을 닫는 전체 마침표 중괄호입니다. 절대 지우거나 이 밑에 코드를 적지 마세요!
    ...
    private IEnumerator MoveBlockSmoothlyUI(GameObject target, Vector2 targetPosition)
    {
        ...
        float elapsed = 0f;
        float duration = 0.15f; // 0.2초 동안 부드럽게 이동
